...
print("Here")
counter = 0
# for x,y in get_data_ratio_wild(files_perm_val, type_dict_val, files_val, input_shape, batch_size):
#     print(counter + 1)
#     counter+=1
#     # pass
# pu.db
...
# pu.db
if tv == 'v' or tv == 'V':
    print(model_gpu.evaluate_generator(get_data_ratio_wild(files_perm_val, type_dict_val, files_val, input_shape, batch_size, 'v'), steps=(num_validation_samples * 2 // (batch_size * 4)), use_multiprocessing=True, workers=16, max_queue_size=32))
else:
    for _ in xrange(EPOCHS):
        print (_)
        # if _ % 50 == 0:
        #     for x in get_data_ratio_wild(files_perm):
        #       print "Prediction: "
        ...
        acc = model_gpu.fit_generator(generator=get_data_ratio_wild(files_perm, type_dict, files, input_shape, batch_size),
                                            steps_per_epoch=(num_training_samples * 2 // (batch_size * 4)),
                                            epochs=1,
                                            verbose=1,
                                            validation_data=get_data_ratio_wild(files_perm_val, type_dict_val, files_val, input_shape, batch_size,tv = 'v'),
        ...
        if acc.history['accuracy'][0] > acc_hist:
            print("Saving model")
            model.save_weights("check_weights_distance_wild.h5")
